...
func (p pair) isFullyIntersected(otherPair pair) bool {

	if p.startPoint >= otherPair.startPoint && p.endPoint <= otherPair.endPoint {
		return true
	}
...
func solution(input []string) {
	...
	for i := 0; i < len(input); i++ {
		...
		if firstPair.isFullyIntersected(secondPair) || secondPair.isFullyIntersected(firstPair) {
			result++
		}
	}
